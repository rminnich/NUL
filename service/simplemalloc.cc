...
void *memalloc_mempool(unsigned long size, unsigned long align)
{
  ...
  unsigned old, _new;
  do {
    old = reinterpret_cast<unsigned long>(s);
...
void * operator new(size_t size)   { return memalloc(size, 0); }
void * operator new[](size_t size) { return memalloc(size, 0); }
void * operator new[](size_t size, unsigned alignment) { return memalloc(size, alignment); }
void * operator new(size_t size, unsigned alignment) { return memalloc(size, alignment); }
